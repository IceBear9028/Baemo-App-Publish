...
export async function fetchUploadNoticeImage({images}: UploadImagesReq): Promise<ReqImageList[] | undefined> {
  try {
    ...
    if (uploadImageCount) {
      const {data} = await apiRequest<{urls: ReqImageUrls[]}, {count: number}>('api/notice/images/path', 'post', {
